...
function roulette_selection(population, selection_rate=0.5)
    # use inverse fitness as the total fitness
    total_fitness = sum(1.0 ./ fitness.(population))
    selected = []
    ...
    while length(selected) < length(population) * selection_rate
        ...
        for individual in population
            accumulator += 1.0 / fitness(individual)
...
function determine_minimum_fitness(population)
    minimum_fitness = 10000000000
    best_route = population[1]
    best_individual_index = 1
    i=1
    for individual in population
        fitness_value = fitness(individual)
        if fitness_value < minimum_fitness
            minimum_fitness = fitness_value
            best_route = individual
            best_individual_index = i
        end
        i += 1
    end
    return best_individual_index, best_route, minimum_fitness
end
...
function genetic_algorithm(coordinates, generations, population_size = 100, selection_rate=0.3, mutation_rate=0.05)
    ...
    for index in 1:generations
        ...
        _, population_best_route, population_best_fitness = determine_minimum_fitness(population)
